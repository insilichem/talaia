...
For information on the visual code see: https://doi.org/10.1101/2022.11.28.517961
...
Talaia expects a Chimera selection to work with. If none is provided, by default it will select the ligands present, if any, in the model and every residue within 8A.
...
TALAIA is used to depict the inner residues of the pore of a potassium channel as presented in the corresponding manuscript (PDB code: [6eo1](https://www.rcsb.org/structure/6eo1)).
...
We are now going to visualize a Vitamin D receptor (PDB code: [5xzf](https://www.rcsb.org/structure/5XZF)).
...
- We also would like to change the representation of the ligand. To do so we select the ligand with `sel :8j0` and `represent sphere sel`.
...
- With the ligand still selected, run the command `sel sel zr < 5` to select every residue that is found at less than 5A from the ligand.
...
- If you want you can hide the ribbon representation. This way it is very easy to see that the binding site physicochemical characteristics complement the ligand structure. To do so you have to run the command `show :8j0`, only the ligand atoms will be represented now. Run `~ribbon` to hide the ribbon from the whole model.
...
To see this, we are going to use a Cu/Zn Dismutase (PDB code: [1to5](https://www.rcsb.org/structure/1TO5)).
...
In TALAIA, the non-standard residues bonded covalently to the backbone of the protein will be represented with a star shape placed on top of the Cα. 
The rest of the small molecules often present in PDB structures, like ligands, solvents, etc. will be ignored.
To exemplify the treatment of non-standard residues we are going to use as example the PDB structure of allophycocyanin. (PDB code: [6yx7](https://www.rcsb.org/structure/6YX7))
...
- Run `sel sel zr < 5` to select all the residues in proximity of 5A from the original selection.
...
TALAIA can be used for MD trajectories aswell. To exemplify how to do it we are going to use a trajectory where [insulin is interacting with oxaliplatin](https://doi.org/10.1039/c8mt00341f). Due to this interaction one of the disulphide bridges of the insulin is broken, thus allowing structural changes to the protein structure. With TALAIA we can visualize the forces driving the conformational change along the trajectory.

To use TALAIA for MD trajectories:
...
If you execute TALAIA from the command line with a **fixed selection**, as we did in the previous examples, the figures will change positions and orientations with the protein structure. 

If you would like to have a **dynamic selection** you can use the Per-Frame tool.
...
**REMEMBER**: You must specify the selection you want to represent.
```
...
talaia spec sel
```
It is important that you remove TALAIA's representation from preovious frames. If you don't, the models will accumulate unnecessarily clutering the view and consuming resources.

In this example, as the trajectory moves forward we will see represented the residues near the ligand at each frame. We will be able to identify interatctions between the ligand and the protein and their evolution through time.
